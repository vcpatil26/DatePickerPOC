...
(function (module) {
    ...
    module.run(['$templateCache', function ($templateCache) {
        $templateCache.put('ionic-multi-date-picker-modal.html',
            '<ion-modal-view class=ionic-datepicker><ion-header-bar class=title ng-class=headerClass><div ng-class=headerClass ng-bind=header></div></ion-header-bar><ion-content><div><ng-include src="\'template-body.html\'"></ng-include></div></ion-content><ion-footer-bar ng-class=modalFooterClass><div class="row no-padding btns-right-modal"><button class=button ng-repeat="btn in btns" ng-click=btn.onClickModal() ng-disabled="isDisabled" ng-class=btn.type>{{ btn.text }}</button></div></ion-footer-bar></ion-modal-view>');
    }]);
})();
            ...
            link: function (scope, element, attrs) {
                ...
                function initDates() {
                    ...
                    if (scope.selectedDates) {
                        scope.inputObj.selectedDates = angular.copy(scope.selectedDates);
                        scope.inputDates = angular.copy(scope.selectedDates);
                            ...
                            if (this.length !== 2) {
                                ...
                                for (var i = 0; i < this.length - 1; i++) {
                                    if (this[i + 1].sortField - this[i].sortField !== 1) {
                                        ...
                                        var diff = (d2.getTime() - d1.getTime()) / (24 * 60 * 60 * 1000) + 1;
                        ...
                        if (scope.selectedDates.length === 0) {
                            ...
                            if (scope.selectedDates.length > 2) {
                                    Array.prototype.splice.call(
                                        scope.selectedDates,
                                        0,
                                        (scope.selectedDates.length - 1)
                                        );
                                
                            }
                            for (var m = 0; m < this.length; m++) {
                                this[m].style.isSelected = false;
                            }
                            if (scope.selectedDates.length === 1) {
                                for (var n = 0; n < this.length; n++) {
                                    this[n].style.isSelected = false;                       //Resetting the existing styles for the calendar elements
                                }
                            }
                            var selectedMonths = scope.selectedDates[0].getMonth();
                            var selectedDates = scope.selectedDates[0].getDate();
                            var selectedYears = scope.selectedDates[0].getFullYear();
                            var k = this.findDay(selectedYears, selectedMonths, selectedDates);
                            if (typeof k != "undefined") {
                                this[k].style.isSelected = !this[k].style.isSelected;
                            }
                        }
                        if (scope.selectedDates.length === 2) {
                            scope.selectedDates.sortByDate();
                            var d1 = new Date(scope.selectedDates[0].getFullYear(), scope.selectedDates[0].getMonth(), scope.selectedDates[0].getDate());
                            var d2 = new Date(scope.selectedDates[1].getFullYear(), scope.selectedDates[1].getMonth(), scope.selectedDates[1].getDate());
                            scope.selectedDatesRange = getDates(d1, d2);
                            var firstDay = glueDate(this[0]);
                            var lastDay = glueDate(this[this.length - 1]);

                            for (var i = 0; i < scope.selectedDatesRange.length; i++) {
                                var selectedDays = glueDate(scope.selectedDatesRange[i]);
                                if (selectedDays >= firstDay && selectedDays <= lastDay) {
                                    viewMonthDates.push(scope.selectedDatesRange[i]);
                                } else if (selectedDays > lastDay) {
                                    break;
                                }
                            }

                            for (var j = 0; j < (viewMonthDates.length); j++) {
                                var selectedMonth = viewMonthDates[j].getMonth();
                                var selectedDate = viewMonthDates[j].getDate();
                                var selectedYear = viewMonthDates[j].getFullYear();
                                var f = this.findDay(selectedYear, selectedMonth, selectedDate);
                                if (this[f].style.isSelected == true) {
                                    this[f].style.isSelected = this[f].style.isSelected;
                                } else {
                                    this[f].style.isSelected = !this[f].style.isSelected;
                                }
                            }
                ...
                function refreshDateList() {
                    ...
                    for (var j = 0; j < insertDays; j++) {
                        ...
                        for (c = 0; c < 8; c++) {
                            isCalendar[c] = scope.selectedDates.findDate.call(scope.calendar[c].dates, date.year, date.month, lastDay).isPresent;
                        }
